#! /usr/bin/python
#
# Automated Inept Epub
#
# aineptepub.pyw, version 4.1
...
class DecryptionDialog(Tkinter.Frame):
    def __init__(self, root):
        ...
        self.status = Tkinter.Label(self, text='Automated Inept ePub V4.1 - Greetings to ILC!')
        ...
        self.keypath = Tkinter.Entry(body, width=35)
        self.keypath.grid(row=0, column=1, sticky=sticky)
        if os.path.exists('adeptkey.der'):
            self.keypath.insert(0, 'adeptkey.der')
        button = Tkinter.Button(body, text="...", command=self.get_keypath)
        button.grid(row=0, column=2)
        Tkinter.Label(body, text='Input Path').grid(row=1)
        self.inpath = Tkinter.Entry(body, width=35)
        self.inpath.grid(row=1, column=1, sticky=sticky)
        button = Tkinter.Button(body, text="...", command=self.get_inpath)
        button.grid(row=1, column=2)
        Tkinter.Label(body, text='Output Path').grid(row=2)
        self.outpath = Tkinter.Entry(body, width=35)
        ...
        botton = Tkinter.Button(
            buttons, text="Decrypt whole directory", width=35, command=self.decrypt)
        ...
        for fname in dirlist:
             ...
             if re.match(match,ext,re.IGNORECASE):
                ...
                try:
                    ...
                    return
                self.status['text'] = 'Decrypted directory '+inpath
...
def gui_main():
    ...
    if RSA is None:
        ...
        tkMessageBox.showerror(
            ...
            "separately.  Read the top-of-script comment for details.")
        return 1
    root.title('Automated INEPT ePub Decrypter')
